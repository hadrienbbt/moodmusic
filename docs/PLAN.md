...
| 3 Auth | done | #4 | Authorization Code flow run by the server, Firestore session store (appendix G), users created or updated at login with the top 15 artists imported the first time, GET /api/session, logout, Origin check, "Compte non autorisé" page, Firestore failures as 503; a real login with Hadrien's account verified in development through http://127.0.0.1:5173 on 2026-09-23 (user, 15 artists and session stored); the Spotify client only has the login calls (refresh and backoff in step 4); refreshStaleArtists comes with step 5 |
| 4 Spotify + ReccoBeats clients | done | #5 | Spotify client of appendix I (refresh at expiresAt − 60 s, once per session for parallel calls; invalid_grant and 401 → ReauthError; 429 backoff 1/2/4 s with Retry-After up to 8 s; SpotifyError with Spotify's message, status 0 for timeouts; the eight helpers with their operation ids); ReccoBeats client (batches of 40 split on a 400, 24 h cache for 500 artists, recommendations never cached); fakes for both; probes moved onto the clients; artist lookups are also limited to 40 ids, and recommendations often return fewer tracks than asked (docs/spotify-capabilities.md) |
| 5 Artists API | done | #6 | GET/POST /api/me/artists, import-top, PUT …/moods, DELETE, GET /api/moods; users repository with addArtist, removeArtist, setArtistMoods, mergeTopArtists and refreshStaleArtists (7 days, at most 30 per login, oldest first; a 404 only drops the image), in transactions; the §4.5 mapping of Spotify and ReccoBeats failures (server/upstream-errors.js) moved here from step 6 because these endpoints already call Spotify; upsertOnLogin stands for createFromProfile and touchLogin, as in appendix H |
| 6 Engines + playlists | done | #7 | ReccoBeats engine (appendix J; the top-up is filtered by MARKET too), the V1 Spotify engine behind RECOMMENDER=spotify, POST /api/playlists and GET /api/me/playlists with the history in Firestore, the no-seed 422; manual check on 2026-09-25 with Hadrien's account in development: 5 seed artists, 4 seeds found (one artist unknown to ReccoBeats), 30 tracks (25 recommended, 5 topped up), about 6 s with a cold ReccoBeats cache |
| 7 Client | todo | | |
| 8 Beta deploy | todo | | |
| 9 Beta | todo | | |
| 10 Cutover | todo | | |
